...
async function createMarkdownDocument(markdown: string): Promise<string> {
  try {
    ...
    const filename = `react-inp-audit-${timestamp}.md`;
...
Usage:
  react-inp-audit analyze [options]
...
Examples:
  react-inp-audit analyze --diff main..HEAD
  react-inp-audit analyze --files src/App.tsx,src/Button.tsx
  react-inp-audit analyze --diff HEAD~1..HEAD --fail-on-high
`);
}
